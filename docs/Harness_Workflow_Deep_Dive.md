...
- `tests/test_dynamic_context_suite.py`
...
+-- tests/                      contracts、gates、controller 的 regression tests
...
| WF4 | `data-prep` | 数据集统计和数据假设 | 数据集事实必须可记录、可复现 |
| WF5 | `baseline-repro` | 基线报告和合同 | 已批准的 baseline/evaluation contract readiness |
| WF6 | `refine-arch` | 技术规格 | 架构必须遵守证据和合同 |
| WF7 | `build-plan` | 实施路线图和 `project_map.json` | plan 不能发明架构；map 必须保持最新 |
| WF8 | `code-expert` | 首轮实现 | stable code 需要先读取 project map 并验证 |
| WF9 | `validate-run` | 语义审查和 smoke evidence | 没有 review 和 smoke evidence 不能 PASS |
...
iterate / auto-iterate
  ...
  +-- 写入 docs/iterations/* 或 docs/40_iterations/*
...
eval
  ...
  +-- git_commit
  +-- run_manifest
  +-- docs/iterations/ 或 docs/40_iterations/ 下的 per-iteration report
```
...
pytest tests/test_dynamic_context_suite.py
pytest tests/test_protocol_compiler.py tests/test_protocol_drift_check.py
pytest tests/test_review_packet.py
pytest tests/test_evidence_docchain.py tests/test_evidence_docchain_gates.py
pytest tests/test_auto_iterate_controller_fsm.py
pytest tests/test_auto_iterate_runtime_adapter.py
```
...
python tooling/evidence/check_dynamic_context.py --workspace-root . --stage status
pytest tests/test_dynamic_context_suite.py
pytest tests/test_dynamic_context_contracts.py
pytest tests/test_evidence_docchain.py tests/test_evidence_docchain_gates.py
```
...
```bash
pytest tests/test_auto_iterate_goal_parser.py
pytest tests/test_auto_iterate_controller_fsm.py
pytest tests/test_auto_iterate_runtime_adapter.py
pytest tests/test_auto_iterate_recovery.py
```
...
code
  -> 按 roadmap/project_map 实现或修复；代码事实要靠测试和运行证明
...
| 收集研究证据 | 有研究问题、论文、repo、数据或 metric 需要记录 | 填 `docs/30_evidence/**` | evidence 是材料，不是 approved contract |
...
| 写代码 | 已有 refined idea、technical spec、roadmap 或明确 bug | `$code-expert` / `$code-debug` | stable code 前读取 `project_map.json`；不要跳过验证 |
