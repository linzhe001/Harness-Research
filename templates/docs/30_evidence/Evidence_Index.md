# Evidence Index

Status: current

## Current Answer

Index the evidence collected for this project. This file records evidence, not
approved rules.

## Evidence Tables

- `docs/30_evidence/Paper_Table.md`
- `docs/30_evidence/Repo_Table.md`
- `docs/30_evidence/Dataset_Table.md`
- `docs/30_evidence/Baseline_Table.md`
- `docs/30_evidence/Validation_Table.md`
- `docs/30_evidence/Metric_Table.md`
- `docs/30_evidence/Open_Questions.md`

## Evidence Summary

| ID | Kind | Source | Used For | Confidence |
|---|---|---|---|---|
| E001 | paper/repo/dataset/benchmark/metric | | | low/medium/high |
