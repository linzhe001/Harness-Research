...
| `harness <action>` / visible `$prepare|$build|$run|$analyze|$write|$change` aliases | intent 已经能进入执行、验证、迭代、write，或成熟代码库出现新需求 | `workflow_ctl status --json`、worker result JSON、Gate ledger |

当 operator 确认 Grill draft 已经足够进入下一步时，agent 应直接继续内部
`init-project update-from-grill` mode，用 Grill 产出的 Research Intent Draft、
...
frontmatter 以通过 loader 扫描，但通过每个 internal skill 下的
`agents/openai.yaml` product policy 从 Codex 可见技能列表中过滤掉。其中
`prepare/build` 会路由到
...
| --- | --- | --- |
| `prepare` | intent 已有，需要 approved readiness、数据/baseline 准备和 Review Packet | pending request JSON / Review Packet |
...
```text
OPERATOR_CONTEXT.md
  -> docs/context/evidence.md Conclusion Evidence index
  -> docs/context/protocol.md Protocol Draft
  -> docs/context/contracts.md draft/approved contracts
  -> .evidence/chains/**
  -> Review Packet
...
Harness 不鼓励让 AI 在不了解代码库的情况下直接生成大补丁。稳定实现前，相关 Skill 会读取 `project_map.json`、`docs/context/facts.md`、roadmap、glossary、contracts 和当前代码路径；实现后再通过 `$code-review`、`$validate-run`、测试命令和 Gate Ledger 把风险显式化。
...
WF10 iteration_log.json / docs/context/experiments.md (legacy mirror: docs/40_iterations/**, docs/iterations/**)
...
`docs/context/experiments.md` 是 WF10 的 durable backlog 和 Research Wiki：记录下一轮实验问题、falsifier、control、paper-driven run request、Assurance Axis 缺口、observations、method notes、paper context、open questions 和已被 Source Artifact 支撑的 findings；它不是 Approved Contract。旧 `docs/40_iterations/Experiment_Queue.md` 和 `docs/45_discoveries/Research_Wiki.md` 仍可由迁移工具作为 legacy inputs 合并进 v2。
...
| `.agents/skills/**` | Codex-facing visible aliases plus internal Skill instruction sources. Internal sources keep minimal frontmatter for loader hygiene and use per-skill `agents/openai.yaml` product policy files so only visible aliases appear in `$` autocomplete. |
...
| `tooling/run_health/**` | Notification-free watchdog and pollable long-run health status. |
...
python tooling/evidence/compile_doc.py --workspace-root . --doc docs/context/contracts.md --source PROJECT_STATE.json docs/context/evidence.md
...
- `docs/context/*.md` stores canonical dynamic-context-v2 docs.
- Legacy `docs/30_evidence/**` stores operator-readable Conclusion Evidence
  tables only until migrated with `migrate_context_v2.py`.
...
prepare completes as `prepare_complete` once readiness, acquisition, workers,
protocol compilation, and the WF5 gate pass; only missing inputs, policy
blockers, worker failures, or gate failures pause for HITL.
...
| Grill or supervisor update | `.agents/references/workflow-supervisor-runtime.md`, `.agents/skills/grill/SKILL.md`, `.agents/skills/workflow-supervisor/SKILL.md`, `tooling/workflow_supervisor/**`, `tooling/.tests/test_workflow_supervisor*.py` |
