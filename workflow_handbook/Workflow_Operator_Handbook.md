...
现在想推进哪类工作、应该从哪个 human-facing Entrypoint 启动、怎么看状态、
什么时候必须停下来让人决策。
...
- [[page:operator_task_index|Operator Task Index]]：按“我想做什么”选择入口、状态命令和停点。
- [[page:workflow_supervisor_model|Workflow Supervisor Model]]：理解 Grill、prepare/build/iterate/release、change 的边界。
...
| 你现在想做什么 | 入口 | 先看哪里 |
| --- | --- | --- |
| 把粗糙 idea 问清楚 | `harness grill` / `$grill` | Research Intent Draft、Grill Round Log |
| 判断能否进入执行 | `harness prepare --dry-run` | readiness preflight、Review Packet |
| 推进 build / validate | `harness build` | worker result JSON、Gate ledger |
| 跑多轮实验 | `harness iterate` | `auto_iterate_ctl.sh status --json`、`iteration_log.json` |
| 处理成熟代码库的新需求 | `harness change` | Change Request JSON、route confidence |
| 做 release / claim gate | `harness release --action ...` | WF12 Review Packet、Claim Boundary |
| 排查内部失败 | Detailed Reference | Stage Reference、Skill Reference |

完整索引见 [[page:operator_task_index|Operator Task Index]]。
...
| Entrypoint | 什么时候用 | 它不会做什么 |
| --- | --- | --- |
| `harness grill` / `$grill` | 研究意图还不清楚，需要追问、挑战、收敛 Research Intent | 不批准 contract，不宣称 WF1-WF3 完成 |
| `harness prepare` / `$workflow-supervisor` | 已有 intent，需要验证 readiness、Review Packet 和 contract approval plumbing | v0 的 `prepare_hitl_poc` 不解锁 build/iterate/release |
| `harness build` / `$workflow-supervisor` | 需要在 approved boundaries 内推进 build/validate orchestration | 不绕过 Stage Skill，不信任 worker prose |
| `harness iterate` / `$workflow-supervisor` | 需要把 WF10 交给 auto-iterate controller 并监控 halt reason | 不手写 `.auto_iterate/**`，不替代 WF10 goal approval |
| `harness release` / `$workflow-supervisor` | 需要 release/claim approval gate 和 exact scoped action | 不 package/submit，除非 exact approved action 已被明确批准 |
| `harness change` / `$change-intake` | codebase 已建立后出现新需求、新 idea、config/code delta | 不默认重跑完整 workflow，不直接编辑代码或 contract |
...
```text
operator request
  -> choose human-facing Entrypoint
...
不要把 generated HTML、hover card、Review Packet 或 hook notice 当成批准。
它们只是阅读视图、decision input 或 guardrail signal。
...
- [[page:workflow_layers|Detailed Workflow Map]]：Intent、Entrypoint、内部 Stage/Skill/Gate 的关系。
...
- [[page:workflow_terms|Workflow Terms]]：Entrypoint、Pending Request、Gate Evidence 等术语。
