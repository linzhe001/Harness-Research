...
- [[page:operator_task_index|Operator Task Index]]：按“我想做什么”选择顶层入口、supervisor action、状态命令和停点。
...
| 准备数据集和 baseline | Execution Supervisor | `prepare --complete` | Dataset Stats、Baseline Report、Review Packet |
| 判断能否进入执行 | Execution Supervisor | `prepare --dry-run` | readiness preflight、Review Packet |
| 推进 build / validate | Execution Supervisor | `build --auto` 或 `build --worker-command ...` | worker result JSON、Gate ledger、Validate Run Report |
| 跑多轮实验 | Execution Supervisor | `iterate` | `auto_iterate_ctl.sh status --json`、`iteration_log.json` |
| 处理成熟代码库的新需求 | Execution Supervisor | `change` | Change Request JSON、route confidence |
| 做 release / claim gate | Execution Supervisor | `release --action ...` | WF12 Review Packet、Claim Boundary |
...
| `harness <action>` / `$workflow-supervisor` | intent 已经能进入执行、验证、迭代、release，或成熟代码库出现新需求 | `prepare`、`build`、`iterate`、`release`、`change` actions | 不绕过 Gate，不信任 worker prose，不替人批准 contract / claim / release |
...
`prepare --complete` 会在输入明确时验证或获取数据集、clone 或采用 baseline、
...
`build --auto` 或 `build --worker-command ...` 会按 build registry 推进
