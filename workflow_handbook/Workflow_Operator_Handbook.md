...
- 需要理解 supervisor entrypoints 时读 [[page:workflow_supervisor_model|Workflow Supervisor Model]]。
...
| Entrypoint | 这次是手动 Skill、Grill、supervisor segment，还是 change intake？ | `$grill`, `$workflow-supervisor`, `$change-intake`, Stage Skills |
...
- [[page:workflow_supervisor_model|Workflow Supervisor Model]]：Grill、Execution Supervisor、Change Intake 的运行边界。
