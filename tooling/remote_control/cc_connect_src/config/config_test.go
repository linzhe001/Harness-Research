...
func TestProviderConfig_AddAndRemove(t *testing.T) {
	...
	got := cfg.Projects[0].Agent.Providers[2]
	if got.Priority != 50 || got.CooldownSec != 900 {
		t.Fatalf("provider metadata = %+v, want priority/cooldown persisted", got)
	}
	if got.AutoFailover == nil || *got.AutoFailover {
		t.Fatalf("provider auto_failover = %v, want false", got.AutoFailover)
	}
