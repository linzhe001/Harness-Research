...
type Engine struct {
	...
	tts                   *TTSCfg
	display               DisplayCfg
	harnessCards          HarnessCardCfg
	providerFailover      *providerFailoverRuntime
	defaultQuiet          bool
	injectSender          bool
...
func NewEngine(name string, ag Agent, platforms []Platform, sessionStorePath string, lang Language) *Engine {
	...
	e := &Engine{
		...
		providerFailover:      newProviderFailoverRuntime(),
		startedAt:             time.Now(),
...
// SetHarnessCardConfig overrides Harness-specific card presentation settings.
func (e *Engine) SetHarnessCardConfig(cfg HarnessCardCfg) {
	e.harnessCards = cfg
}
...
func (e *Engine) getOrCreateWorkspaceAgent(workspace string) (Agent, *SessionManager, error) {
	...
	// Copy reasoning effort
	if ra, ok := e.agent.(interface{ GetReasoningEffort() string }); ok {
		if effort := ra.GetReasoningEffort(); effort != "" {
...
func (e *Engine) getOrCreateInteractiveStateWith(sessionKey string, p Platform, replyCtx any, session *Session, sessions *SessionManager, agentOverride Agent) *interactiveState {
	...
	if e.ctx.Err() != nil {
		...
		state = &interactiveState{agent: agent, platform: p, replyCtx: replyCtx, quiet: quietMode}
...
func (e *Engine) processInteractiveEvents(state *interactiveState, session *Session, sessions *SessionManager, sessionKey string, msgID string, turnStart time.Time, stopTypingFn func()) {
	...
	for {
		...
		case EventError:
			...
			if event.Error != nil {
				if e.tryAutoSwitchProvider(state, session, sessions, sessionKey, event.Error) {
					return
				}
...
type harnessHomeEvent struct {
	TS         string `json:"ts"`
	Event      string `json:"event"`
	RoundIndex any    `json:"round_index"`
	PhaseKey   string `json:"phase_key"`
}

type harnessHomeSummaryEnvelope struct {
	OK       bool                   `json:"ok"`
	Category string                 `json:"category"`
	Message  string                 `json:"message"`
	ExitCode int                    `json:"exit_code"`
	Data     harnessHomeSummaryData `json:"data"`
}

type harnessHomeSummaryData struct {
	Workspace struct {
	...
	StagedGoalPresent  bool                `json:"staged_goal_present"`
}

func (e *Engine) cmdHome(p Platform, msg *Message) {
	e.replyWithCard(p, msg.ReplyCtx, e.renderHomeCard(msg.SessionKey))
}

func (e *Engine) workspaceDirForSessionKey(sessionKey string) string {
...
func (e *Engine) runHarnessRemoteJSON(workspaceDir string, args ...string) ([]byte, error) {
	script := filepath.Join(workspaceDir, "tooling", "remote_control", "scripts", "harness_remote.sh")
	if _, err := os.Stat(script); err != nil {
		return nil, fmt.Errorf("harness remote wrapper not found: %s", script)
	}
	...
	if ctx.Err() == context.DeadlineExceeded {
		return nil, fmt.Errorf("harness remote command timed out")
	}
	if err != nil {
		...
		return nil, fmt.Errorf("%s", truncateStr(msg, 600))
	}
	return output, nil
}

func (e *Engine) loadHarnessHomeSummary(sessionKey string) (*harnessHomeSummaryEnvelope, error) {
	workspaceDir := e.workspaceDirForSessionKey(sessionKey)
	output, err := e.runHarnessRemoteJSON(workspaceDir, "summary", "--json")
	if err != nil {
		return nil, err
	}

	var envelope harnessHomeSummaryEnvelope
	if err := json.Unmarshal(output, &envelope); err != nil {
		return nil, fmt.Errorf("parse harness summary: %w", err)
	}
	return &envelope, nil
}
...
func (e *Engine) homeLabel(en, zh string) string {
...
func (e *Engine) agentForSessionKey(sessionKey string) Agent {
	agent, _ := e.sessionContextForKey(sessionKey)
	if agent != nil {
		return agent
	}
...
func (e *Engine) harnessCardsEnabled() bool {
	return boolOrDefault(e.harnessCards.Enabled, true)
}

func (e *Engine) harnessHomeTitle() string {
	if e.harnessCardsEnabled() {
		if title := strings.TrimSpace(e.harnessCards.HomeTitle); title != "" {
			return title
		}
	}
	return e.i18n.T(MsgCardTitleHome)
}

func (e *Engine) harnessShowWorkspaceRoot() bool {
	return boolOrDefault(e.harnessCards.ShowWorkspaceRoot, true)
}

func (e *Engine) harnessShowMetric() bool {
	return boolOrDefault(e.harnessCards.ShowMetric, true)
}

func (e *Engine) harnessShowAccount() bool {
	return boolOrDefault(e.harnessCards.ShowAccount, true)
}

func (e *Engine) harnessShowRecommendedActions() bool {
	return boolOrDefault(e.harnessCards.ShowRecommendedActions, true)
}

func (e *Engine) harnessShowCopyableCommands() bool {
	if !e.harnessCardsEnabled() {
		return false
	}
	return boolOrDefault(e.harnessCards.ShowCopyableCommands, true)
}

func (e *Engine) harnessShowProviderCopyableCommands() bool {
	if !e.harnessCardsEnabled() {
		return false
	}
	return boolOrDefault(e.harnessCards.ProviderCardShowCopyableCommands, e.harnessShowCopyableCommands())
}

func (e *Engine) harnessShowModelCopyableCommands() bool {
	if !e.harnessCardsEnabled() {
		return false
	}
	return boolOrDefault(e.harnessCards.ModelCardShowCopyableCommands, e.harnessShowCopyableCommands())
}

func (e *Engine) harnessShowReasoningCopyableCommands() bool {
	if !e.harnessCardsEnabled() {
		return false
	}
	return boolOrDefault(e.harnessCards.ReasoningCardShowCopyableCommands, e.harnessShowCopyableCommands())
}

func (e *Engine) harnessInteractionHint() string {
	if !e.harnessCardsEnabled() {
		return ""
	}
	if hint := strings.TrimSpace(e.harnessCards.InteractionHint); hint != "" {
		return hint
	}
	return e.i18n.T(MsgCardImmediateApplyHint)
}

func (e *Engine) cardCommandFallbackMarkdown(commands []string) string {
	seen := make(map[string]bool, len(commands))
	lines := []string{fmt.Sprintf("**%s**", e.i18n.T(MsgCardCopyCommandsTitle))}
...
func (e *Engine) appendCardCommandFallback(cb *CardBuilder, commands []string) {
	if markdown := e.cardCommandFallbackMarkdown(commands); markdown != "" {
		cb.Divider().Markdown(markdown)
	}
}

func (e *Engine) appendStackedButtons(cb *CardBuilder, buttons ...CardButton) {
...
func (e *Engine) cmdHelp(p Platform, msg *Message, args []string) {
	...
	if len(args) > 0 {
		group = resolveHelpGroupKey(args[0])
	}
	...
	if !supportsCards(p) {
		...
		e.reply(p, msg.ReplyCtx, e.renderHelpGroupCard(group).RenderText())
		return
	}
	e.replyWithCard(p, msg.ReplyCtx, e.renderHelpGroupCard(group))
}
...
func helpCardGroups() []helpCardGroup {
	return []helpCardGroup{
		{
			key:      "workspace",
			titleKey: MsgHelpWorkspaceSection,
			items: []helpCardItem{
				{command: "/home", action: "nav:/home"},
		...
		{
			key:      "auto",
			titleKey: MsgHelpAutoSection,
			items: []helpCardItem{
				{command: "/ai status", action: "cmd:/ai status", description: "查看 auto-iterate 当前状态 / Show loop status"},
				{command: "/ai tail", action: "cmd:/ai tail", description: "查看最近 controller 事件 / Show recent controller events"},
				{command: "/ai pause", action: "cmd:/ai pause", description: "在安全边界暂停 loop / Pause at next safe boundary"},
				{command: "/ai resume", action: "cmd:/ai resume", description: "恢复当前 loop / Resume current loop"},
				{command: "/ai stop", action: "cmd:/ai stop", description: "在安全边界停止 loop / Stop at next safe boundary"},
				{command: "/ai logs", action: "cmd:/ai logs", description: "查看最近运行日志 / Show recent runtime logs"},
				{command: "/ai override", action: "cmd:/ai override", description: "提交下一轮 goal override / Stage next-round goal override"},
			},
		},
		{
			key:      "tools",
...
func resolveHelpGroupKey(groupKey string) string {
	switch strings.ToLower(strings.TrimSpace(groupKey)) {
	case "":
		return defaultHelpGroup
	case "workspace", "workspaces", "ws":
		return "workspace"
	case "session", "sessions", "chat":
		return "session"
	case "agent", "provider", "model", "reasoning":
		return "agent"
	case "auto", "ai", "auto-iterate", "auto_iterate", "iterate":
		return "auto"
	case "tool", "tools", "skill", "skills", "command", "commands", "alias":
		return "tools"
	case "system", "sys":
		return "system"
	default:
...
func (e *Engine) renderHelpGroupCard(groupKey string) *Card {
	...
	commandText := func(item helpCardItem) string {
		...
		if desc == "" {
			desc = strings.TrimSpace(e.i18n.T(MsgKey(strings.TrimPrefix(item.command, "/"))))
		}
		if desc == "" {
			return "**" + item.command + "**"
		}
	...
	current := groups[0]
	normalizedGroup := resolveHelpGroupKey(groupKey)
	...
	e.appendStackedButtons(cb, tabs...)
...
func (e *Engine) renderHomeCard(sessionKey string) *Card {
	summary, err := e.loadHarnessHomeSummary(sessionKey)
	if err != nil {
		return e.simpleCard(e.harnessHomeTitle(), "turquoise", err.Error())
	}

	data := summary.Data
	agent := e.agentForSessionKey(sessionKey)
	bodyLines := []string{fmt.Sprintf("**%s**: `%s`", e.homeLabel("Workspace", "工作区"), data.Workspace.Name)}
	if e.harnessShowWorkspaceRoot() {
		bodyLines = append(bodyLines, fmt.Sprintf("**%s**: `%s`", e.homeLabel("Root", "根目录"), data.Workspace.Root))
	}
	bodyLines = append(
		bodyLines,
		fmt.Sprintf("**%s**: `%s`", e.homeLabel("AI Loop", "AI Loop"), fallbackString(data.AutoIterate.Status, "inactive")),
		fmt.Sprintf("**%s**: `%v`", e.homeLabel("Round", "轮次"), fallbackAny(data.AutoIterate.CurrentRoundIndex, "n/a")),
		fmt.Sprintf("**%s**: `%s`", e.homeLabel("Phase", "阶段"), fallbackString(data.AutoIterate.CurrentPhaseKey, "n/a")),
	)
	if ps, ok := agent.(ProviderSwitcher); ok {
		...
		bodyLines = append(bodyLines, fmt.Sprintf("**%s**: `%s`", e.homeLabel("Provider", "Provider"), providerName))
	}
	if ms, ok := agent.(ModelSwitcher); ok {
		modelName := fallbackString(ms.GetModel(), "default")
		bodyLines = append(bodyLines, fmt.Sprintf("**%s**: `%s`", e.homeLabel("Model", "模型"), modelName))
	}
	if rs, ok := agent.(ReasoningEffortSwitcher); ok {
		reasoning := fallbackString(rs.GetReasoningEffort(), "default")
		bodyLines = append(bodyLines, fmt.Sprintf("**%s**: `%s`", e.homeLabel("Reasoning", "推理强度"), reasoning))
	}
	if e.harnessShowAccount() {
		bodyLines = append(bodyLines, fmt.Sprintf("**%s**: `%s`", e.homeLabel("Account", "账号"), fallbackString(data.AutoIterate.SelectedAccountID, "n/a")))
	}
	if e.harnessShowMetric() {
		bodyLines = append(bodyLines, fmt.Sprintf("**%s**: `%s`", e.homeLabel("Metric", "指标"), fallbackString(data.AutoIterate.MetricName, "n/a")))
	}
	bodyLines = append(bodyLines, fmt.Sprintf("**%s**: `%s`", e.homeLabel("Staged Goal", "暂存目标"), e.homeBoolText(data.StagedGoalPresent)))
	if data.LatestEvent != nil && data.LatestEvent.Event != "" {
		bodyLines = append(
			bodyLines,
			fmt.Sprintf(
				"**%s**: `%s` (R%v %s)",
				e.homeLabel("Latest Event", "最近事件"),
				data.LatestEvent.Event,
				fallbackAny(data.LatestEvent.RoundIndex, "n/a"),
				strings.TrimSpace(data.LatestEvent.PhaseKey),
			),
		)
	}

	cb := NewCard().
		Title(e.harnessHomeTitle(), "turquoise").
		Markdown(strings.Join(bodyLines, "\n"))

	e.appendStackedButtons(
		cb,
		DefaultBtn(e.homeLabel("Workspace", "工作区"), "cmd:/workspace"),
		DefaultBtn(e.homeLabel("Session", "会话"), "nav:/help session"),
		DefaultBtn(e.homeLabel("Provider", "Provider"), "nav:/provider"),
		DefaultBtn(e.homeLabel("Help", "帮助"), "nav:/help"),
		DefaultBtn(e.homeLabel("AI Commands", "AI 命令"), "nav:/help auto"),
		DefaultBtn(e.homeLabel("AI Status", "AI 状态"), "cmd:/ai status"),
		DefaultBtn(e.homeLabel("AI Tail", "AI 事件"), "cmd:/ai tail"),
	)
	commands := []string{
		"/workspace",
		"/provider",
	...
	if e.harnessShowRecommendedActions() {
		...
		for _, action := range data.RecommendedActions {
			if action.Value == "" || seenValues[action.Value] {
				continue
			}
			seenValues[action.Value] = true
			commands = append(commands, action.Value)
			actionVal := "cmd:" + action.Value
			btnType := "default"
			if len(actionBtns) == 0 {
				btnType = "primary"
			}
			actionBtns = append(actionBtns, Btn(action.Label, btnType, actionVal))
			if len(actionBtns) >= 4 {
				break
			}
		}
		e.appendStackedButtons(cb, actionBtns...)
	}
	if e.harnessShowCopyableCommands() {
		e.appendCardCommandFallback(cb, commands)
	}
	cb.Note(summary.Message)
	return cb.Build()
}
...
func fallbackAny(value any, fallback string) string {
	if value == nil {
		return fallback
	}
...
func (e *Engine) homeBoolText(v bool) string {
	switch e.i18n.CurrentLang() {
	case LangChinese:
		if v {
			return "是"
...
func (e *Engine) cmdModel(p Platform, msg *Message, args []string) {
	switcher, ok := e.agentForSessionKey(msg.SessionKey).(ModelSwitcher)
	if !ok {
	...
	if len(args) == 0 {
		...
		e.replyWithCard(p, msg.ReplyCtx, e.renderModelCardFor(msg.SessionKey))
		return
	}
...
func (e *Engine) cmdReasoning(p Platform, msg *Message, args []string) {
	switcher, ok := e.agentForSessionKey(msg.SessionKey).(ReasoningEffortSwitcher)
	if !ok {
	...
	if len(args) == 0 {
		...
		e.replyWithCard(p, msg.ReplyCtx, e.renderReasoningCardFor(msg.SessionKey))
		return
	}
...
func (e *Engine) cmdMode(p Platform, msg *Message, args []string) {
	switcher, ok := e.agentForSessionKey(msg.SessionKey).(ModeSwitcher)
	if !ok {
	...
	if len(args) == 0 {
		...
		e.replyWithCard(p, msg.ReplyCtx, e.renderModeCardFor(msg.SessionKey))
		return
	}
...
func (e *Engine) cmdProvider(p Platform, msg *Message, args []string) {
	switcher, ok := e.agentForSessionKey(msg.SessionKey).(ProviderSwitcher)
	if !ok {
	...
	if len(args) == 0 {
		if supportsCards(p) {
			e.replyWithCard(p, msg.ReplyCtx, e.renderProviderCardFor(msg.SessionKey))
			return
		}
	...
	case "clear", "reset", "none":
		...
		if current != nil {
			e.providerFailover.Clear(current.Name)
		}
		e.cleanupInteractiveState(e.interactiveKeyForSessionKey(msg.SessionKey))
		_, sessions := e.sessionContextForKey(msg.SessionKey)
		e.resetConversationState(sessions.GetOrCreateActive(msg.SessionKey), sessions, e.agentForSessionKey(msg.SessionKey))
...
func (e *Engine) cmdProviderAdd(p Platform, msg *Message, switcher ProviderSwitcher, args []string) {
	...
	if strings.HasPrefix(raw, "{") {
		...
		prov = ProviderConfig{
			...
			CooldownSec:  jp.CooldownSec,
			AutoFailover: jp.AutoFailover,
		}
	} else {
...
func (e *Engine) notifyQueuedMessages(state *interactiveState, text string) {
	if strings.TrimSpace(text) == "" || state == nil {
		return
	}
	state.mu.Lock()
	...
	for _, q := range queued {
		e.send(q.platform, q.replyCtx, text)
	}
}

func (e *Engine) tryAutoSwitchProvider(state *interactiveState, session *Session, sessions *SessionManager, sessionKey string, err error) bool {
	if state == nil || err == nil || !IsProviderExhaustionError(err) {
		return false
	}
	...
	current := switcher.GetActiveProvider()
	if current == nil || !providerAutoFailoverEnabled(*current) {
		return false
	}

	now := time.Now()
	e.providerFailover.MarkCooldown(current.Name, now.Add(providerCooldownDuration(*current)))
	next := e.providerFailover.NextProvider(switcher.ListProviders(), current.Name, now)
	if next == nil {
		e.send(p, replyCtx, e.i18n.Tf(MsgProviderAutoUnavailable, current.Name))
		return true
	}
	if !switcher.SetActiveProvider(next.Name) {
		return false
	}

	e.providerFailover.Clear(next.Name)
	if e.providerSaveFunc != nil {
...
func (e *Engine) handleCardNav(action string, sessionKey string) *Card {
	...
	switch cmd {
	case "/home":
		return e.renderHomeCard(sessionKey)
	case "/help":
		return e.renderHelpGroupCard(args)
	case "/model":
		return e.renderModelCardFor(sessionKey)
	case "/reasoning":
		return e.renderReasoningCardFor(sessionKey)
	case "/mode":
		return e.renderModeCardFor(sessionKey)
	case "/lang":
	...
	case "/provider":
		return e.renderProviderCardFor(sessionKey)
	case "/cron":
...
func (e *Engine) executeCardAction(cmd, args, sessionKey string) {
	...
	case "/model":
		...
		switcher, ok := e.agentForSessionKey(sessionKey).(ModelSwitcher)
		if !ok {
			return
		}
	...
	case "/reasoning":
		...
		switcher, ok := e.agentForSessionKey(sessionKey).(ReasoningEffortSwitcher)
		if !ok {
			return
		}
		efforts := switcher.AvailableReasoningEfforts()
	...
	case "/mode":
		...
		switcher, ok := e.agentForSessionKey(sessionKey).(ModeSwitcher)
		if !ok {
			return
		}
	...
	case "/provider":
		...
		switcher, ok := e.agentForSessionKey(sessionKey).(ProviderSwitcher)
		if !ok {
			return
		}
		target := strings.TrimSpace(args)
		if target == "clear" || target == "reset" || target == "none" {
			if current := switcher.GetActiveProvider(); current != nil {
				e.providerFailover.Clear(current.Name)
			}
			target = ""
		}
...
func (e *Engine) renderModelCard() *Card {
	return e.renderModelCardFor("")
}

func (e *Engine) renderModelCardFor(sessionKey string) *Card {
	switcher, ok := e.agentForSessionKey(sessionKey).(ModelSwitcher)
	if !ok {
	...
	cb := NewCard().Title(e.i18n.T(MsgCardTitleModel), "indigo").
		...
		Buttons(e.cardBackButton())
	if hint := e.harnessInteractionHint(); hint != "" {
		cb.Note(hint)
	}
	if e.harnessShowModelCopyableCommands() {
		commands := make([]string, 0, len(models))
		for _, m := range models {
			commands = append(commands, "/model "+m.Name)
		}
		e.appendCardCommandFallback(cb, commands)
	}
...
func (e *Engine) renderReasoningCard() *Card {
	return e.renderReasoningCardFor("")
}

func (e *Engine) renderReasoningCardFor(sessionKey string) *Card {
	switcher, ok := e.agentForSessionKey(sessionKey).(ReasoningEffortSwitcher)
	if !ok {
	...
	if e.harnessShowReasoningCopyableCommands() {
		commands := make([]string, 0, len(efforts))
		for _, effort := range efforts {
			commands = append(commands, "/reasoning "+effort)
		}
		e.appendCardCommandFallback(cb, commands)
	}
...
func (e *Engine) renderModeCard() *Card {
	return e.renderModeCardFor("")
}

func (e *Engine) renderModeCardFor(sessionKey string) *Card {
	switcher, ok := e.agentForSessionKey(sessionKey).(ModeSwitcher)
	if !ok {
	...
	if e.harnessShowCopyableCommands() {
		commands := make([]string, 0, len(modes))
		for _, m := range modes {
			commands = append(commands, "/mode "+m.Key)
		}
		e.appendCardCommandFallback(cb, commands)
	}
...
func (e *Engine) renderProviderCard() *Card {
	return e.renderProviderCardFor("")
}

func (e *Engine) renderProviderCardFor(sessionKey string) *Card {
	switcher, ok := e.agentForSessionKey(sessionKey).(ProviderSwitcher)
	if !ok {
	...
	body.WriteString(e.i18n.T(MsgProviderListTitle))
	for _, prov := range providers {
		marker := "  "
		if current != nil && prov.Name == current.Name {
			marker = "▶ "
	...
	if e.harnessShowProviderCopyableCommands() {
		commands := []string{"/provider list"}
		if current != nil {
			commands = append(commands, "/provider clear")
		}
		for _, prov := range providers {
			commands = append(commands, "/provider switch "+prov.Name)
		}
		e.appendCardCommandFallback(cb, commands)
	}
...
func (e *Engine) executeCustomCommand(p Platform, msg *Message, cmd *CustomCommand, args []string) {
	...
	if cmd.Exec != "" {
		go e.executeShellCommand(p, msg, cmd, args, e.commandWorkspaceDir(msg))
		return
	}
	...
	msg.Content = prompt
	go e.processInteractiveMessageWith(p, msg, session, agent, sessions, interactiveKey, e.commandWorkspaceDir(msg))
}

// executeShellCommand runs a shell command and sends the output to the user.
func (e *Engine) executeShellCommand(p Platform, msg *Message, cmd *CustomCommand, args []string, workspaceDir string) {
...
func (e *Engine) commandWorkspaceDir(msg *Message) string {
	return e.workspaceDirForSessionKey(msg.SessionKey)
}
