...
const (
	...
	MsgProviderAutoSwitched MsgKey = "provider_auto_switched"
	MsgProviderAutoUnavailable MsgKey = "provider_auto_unavailable"
	...
	MsgCardImmediateApplyHint    MsgKey = "card_immediate_apply_hint"
	MsgCardCopyCommandsTitle     MsgKey = "card_copy_commands_title"
...
var messages = map[MsgKey]map[Language]string{
	...
	MsgHelpWorkspaceSection: {
		LangEnglish:            "**Workspace & Home**",
		LangChinese:            "**工作区与工作台**",
		LangTraditionalChinese: "**工作區與工作臺**",
		LangJapanese:           "**ワークスペースとホーム**",
		LangSpanish:            "**Workspace e inicio**",
	},
	...
	MsgHelpAutoSection: {
		LangEnglish:            "**Auto-Iterate Control**",
		LangChinese:            "**自动迭代控制**",
		LangTraditionalChinese: "**自動迭代控制**",
		LangJapanese:           "**自動イテレーション制御**",
		LangSpanish:            "**Control de auto-iterate**",
	},
	MsgHelpToolsSection: {
	...
	MsgProviderAddUsage: {
		LangEnglish: "Usage:\n\n" +
			...
			"Or JSON:\n" +
			"`/provider add {\"name\":\"relay\",\"api_key\":\"sk-xxx\",\"base_url\":\"https://...\",\"model\":\"...\",\"priority\":100,\"cooldown_sec\":1800,\"auto_failover\":true}`",
		LangChinese: "用法:\n\n" +
	...
	MsgProviderAutoSwitched: {
		LangEnglish:            "Quota/rate limit hit on `%s`. Switched to `%s`. Please resend the previous message.",
		LangChinese:            "当前账号 `%s` 已触发配额或限流，已自动切换到 `%s`。请重发上一条消息。",
		LangTraditionalChinese: "當前帳號 `%s` 已觸發配額或限流，已自動切換到 `%s`。請重發上一條訊息。",
		LangJapanese:           "現在のアカウント `%s` がクォータまたはレート制限に達したため、`%s` に切り替えました。直前のメッセージを再送してください。",
		LangSpanish:            "La cuenta `%s` alcanzó el límite de cuota o rate limit. Se cambió a `%s`. Reenvíe el mensaje anterior.",
	},
	MsgProviderAutoUnavailable: {
		LangEnglish:            "Quota/rate limit hit on `%s`, but no fallback provider is currently available. Use `/provider` to inspect or switch manually.",
		LangChinese:            "当前账号 `%s` 已触发配额或限流，但目前没有可用的备用账号。请用 `/provider` 查看或手动切换。",
		LangTraditionalChinese: "當前帳號 `%s` 已觸發配額或限流，但目前沒有可用的備援帳號。請用 `/provider` 查看或手動切換。",
		LangJapanese:           "現在のアカウント `%s` がクォータまたはレート制限に達しましたが、利用可能なフォールバックプロバイダはありません。`/provider` で確認または手動切り替えしてください。",
		LangSpanish:            "La cuenta `%s` alcanzó el límite de cuota o rate limit, pero no hay proveedor de respaldo disponible. Use `/provider` para inspeccionar o cambiar manualmente.",
	},
	MsgVoiceNotEnabled: {
	...
	MsgCardImmediateApplyHint: {
		LangEnglish:            "Selection applies immediately on supported platforms. If card interaction is unavailable, copy one of the commands below.",
		LangChinese:            "在支持的平台上，选择后会立即生效。如果卡片交互不可用，请复制下面的命令执行。",
		LangTraditionalChinese: "在支援的平台上，選擇後會立即生效。如果卡片互動不可用，請複製下面的命令執行。",
		LangJapanese:           "対応プラットフォームでは選択後すぐに反映されます。カード操作が使えない場合は、以下のコマンドをコピーして実行してください。",
		LangSpanish:            "La selección se aplica de inmediato en las plataformas compatibles. Si la interacción con la tarjeta no funciona, copie uno de los comandos de abajo.",
	},
	MsgCardCopyCommandsTitle: {
		LangEnglish:            "Copyable Commands",
		LangChinese:            "可复制命令",
		LangTraditionalChinese: "可複製命令",
		LangJapanese:           "コピー可能なコマンド",
		LangSpanish:            "Comandos para copiar",
	},
	MsgCardBack: {
	...
	MsgCardTitleHome: {
		LangEnglish: "Home", LangChinese: "工作台", LangTraditionalChinese: "工作臺",
		LangJapanese: "ホーム", LangSpanish: "Inicio",
	},
	...
	MsgBuiltinCmdHome: {
		LangEnglish:            "Show workspace home card",
		LangChinese:            "显示工作台卡片",
		LangTraditionalChinese: "顯示工作臺卡片",
		LangJapanese:           "ホームカードを表示",
		LangSpanish:            "Mostrar tarjeta de inicio del workspace",
	},
	MsgBuiltinCmdList: {
