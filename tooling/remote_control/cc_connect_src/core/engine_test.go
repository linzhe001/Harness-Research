...
func TestCustomExecCommand_UsesBoundWorkspaceDirInMultiWorkspace(t *testing.T) {
	p := &stubPlatformEngine{n: "test"}
	e := NewEngine("test", &stubWorkDirAgent{workDir: "/tmp/global"}, []Platform{p}, "", LangEnglish)
	e.SetAdminFrom("admin1")
	...
	e.commands.Add("whereami", "", "", "pwd", "", "config")

	msg := &Message{
		SessionKey: "slack:" + channelID + ":admin1",
		UserID:     "admin1",
		ReplyCtx:   "ctx",
	}
	e.handleCommand(p, msg, "/whereami")

	time.Sleep(300 * time.Millisecond)

	sent := strings.Join(p.getSent(), "\n")
	if !strings.Contains(sent, normalizedWsDir) {
		t.Fatalf("custom exec command should run in bound workspace %q, got %q", normalizedWsDir, sent)
	}
}
...
func TestCmdMode_SwitchesModeAndResetsSession(t *testing.T) {
	p := &stubPlatformEngine{n: "plain"}
	agent := &stubModelModeAgent{}
	e := NewEngine("test", agent, []Platform{p}, "", LangEnglish)
	msg := &Message{SessionKey: "test:user1", ReplyCtx: "ctx"}
	...
	if agent.mode != "yolo" {
		t.Fatalf("mode = %q, want yolo", agent.mode)
	}
	if s.GetAgentSessionID() != "" {
		t.Fatalf("AgentSessionID = %q, want cleared", s.GetAgentSessionID())
	}
	...
	if len(p.sent) != 1 || !strings.Contains(p.sent[0], "Permission mode switched") {
		t.Fatalf("sent = %v, want mode changed message", p.sent)
	}
}
...
func TestRenderHelpCard_DefaultsToSessionTab(t *testing.T) {
	...
	if _, ok := findCardAction(card, "nav:/help auto"); !ok {
		t.Fatal("expected auto help tab to exist")
	}
	if _, ok := findCardAction(card, "nav:/help workspace"); !ok {
		t.Fatal("expected workspace help tab to exist")
	}
}
...
func TestHandleCommand_HelpAutoRendersSpecificGroup(t *testing.T) {
	p := &stubCardPlatform{stubPlatformEngine: stubPlatformEngine{n: "test"}}
	e := NewEngine("test", &stubAgent{}, []Platform{p}, "", LangEnglish)

	e.handleCommand(p, &Message{SessionKey: "test:user1", ReplyCtx: "ctx"}, "/help auto")
	...
	text := p.repliedCards[0].RenderText()
	if !strings.Contains(text, "**/ai status**") {
		t.Fatalf("auto help text = %q, want ai commands", text)
	}
	if strings.Contains(text, "**/new**") {
		t.Fatalf("auto help text = %q, should not include session commands", text)
	}
}

func TestRenderHomeCard_UsesHarnessSummary(t *testing.T) {
	workspaceDir := t.TempDir()
	scriptDir := filepath.Join(workspaceDir, "tooling", "remote_control", "scripts")
	if err := os.MkdirAll(scriptDir, 0o755); err != nil {
		t.Fatal(err)
	}

	summaryJSON := fmt.Sprintf(`{"ok":true,"category":"ok","message":"ready","exit_code":0,"data":{"workspace":{"name":"%s","root":"%s"},"auto_iterate":{"present":true,"status":"paused","halt_reason":"","current_round_index":3,"current_phase_key":"evaluate","selected_account_id":"codex_acc2","metric_name":"pass_rate","best_primary_metric":0.91},"latest_event":{"ts":"2026-03-26T10:00:00Z","event":"round_completed","round_index":3,"phase_key":"evaluate"},"recommended_actions":[{"label":"继续","kind":"command","value":"/ai resume","reason":"resume"}],"staged_goal_present":true}}`, filepath.Base(workspaceDir), workspaceDir)
	script := "#!/usr/bin/env bash\nprintf '%s' '" + strings.ReplaceAll(summaryJSON, "'", "'\"'\"'") + "'\n"
	if err := os.WriteFile(filepath.Join(scriptDir, "harness_remote.sh"), []byte(script), 0o755); err != nil {
		t.Fatal(err)
	}

	e := NewEngine("test", &stubWorkDirAgent{workDir: workspaceDir}, []Platform{&stubPlatformEngine{n: "test"}}, "", LangChinese)

	card := e.renderHomeCard("test:user1")
	text := card.RenderText()

	if !strings.Contains(text, filepath.Base(workspaceDir)) {
		t.Fatalf("home card text = %q, want workspace name", text)
	}
	if !strings.Contains(text, "pass_rate") {
		t.Fatalf("home card text = %q, want metric name", text)
	}
	if !strings.Contains(text, "可复制命令") {
		t.Fatalf("home card text = %q, want copyable commands section", text)
	}
	if !strings.Contains(text, "/ai resume") {
		t.Fatalf("home card text = %q, want recommended resume command", text)
	}
	if _, ok := findCardAction(card, "nav:/help auto"); !ok {
		t.Fatal("expected AI help button on home card")
	}
	if _, ok := findCardAction(card, "cmd:/ai resume"); !ok {
		t.Fatal("expected recommended resume action on home card")
	}
}

func TestRenderHomeCard_UsesHarnessCardConfig(t *testing.T) {
	workspaceDir := t.TempDir()
	scriptDir := filepath.Join(workspaceDir, "tooling", "remote_control", "scripts")
	if err := os.MkdirAll(scriptDir, 0o755); err != nil {
		t.Fatal(err)
	...
	summaryJSON := fmt.Sprintf(`{"ok":true,"category":"ok","message":"ready","exit_code":0,"data":{"workspace":{"name":"%s","root":"%s"},"auto_iterate":{"present":true,"status":"paused","halt_reason":"","current_round_index":3,"current_phase_key":"evaluate","selected_account_id":"codex_acc2","metric_name":"pass_rate","best_primary_metric":0.91},"recommended_actions":[{"label":"继续","kind":"command","value":"/ai resume","reason":"resume"}],"staged_goal_present":true}}`, filepath.Base(workspaceDir), workspaceDir)
	script := "#!/usr/bin/env bash\nprintf '%s' '" + strings.ReplaceAll(summaryJSON, "'", "'\"'\"'") + "'\n"
	if err := os.WriteFile(filepath.Join(scriptDir, "harness_remote.sh"), []byte(script), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	e := NewEngine("test", &stubWorkDirAgent{workDir: workspaceDir}, []Platform{&stubPlatformEngine{n: "test"}}, "", LangChinese)
	e.SetHarnessCardConfig(HarnessCardCfg{
		Enabled:              boolPtr(true),
		HomeTitle:            "Harness 工作台",
		ShowWorkspaceRoot:    boolPtr(false),
		ShowMetric:           boolPtr(false),
		ShowCopyableCommands: boolPtr(true),
	})

	text := e.renderHomeCard("test:user1").RenderText()
	if !strings.Contains(text, "**Harness 工作台**") {
		t.Fatalf("home card text = %q, want custom title", text)
	}
	if strings.Contains(text, workspaceDir) {
		t.Fatalf("home card text = %q, should hide workspace root", text)
	}
	if strings.Contains(text, "pass_rate") {
		t.Fatalf("home card text = %q, should hide metric", text)
	}
	if !strings.Contains(text, "/provider") {
		t.Fatalf("home card text = %q, want copyable command fallback", text)
	}
}

func TestRenderProviderCard_IncludesHintAndCopyableCommands(t *testing.T) {
	agent := &stubProviderAgent{
		providers: []ProviderConfig{
			{Name: "codex_primary", BaseURL: "https://api.openai.com", Model: "gpt-5.4"},
			{Name: "codex_secondary", BaseURL: "https://api.openai.com", Model: "gpt-5.4"},
		},
		active: "codex_primary",
	}
	e := NewEngine("test", agent, []Platform{&stubPlatformEngine{n: "test"}}, "", LangEnglish)

	text := e.renderProviderCard().RenderText()
	if !strings.Contains(text, "Selection applies immediately") {
		t.Fatalf("provider card text = %q, want interaction hint", text)
	}
	if !strings.Contains(text, "/provider switch codex_primary") || !strings.Contains(text, "/provider switch codex_secondary") {
		t.Fatalf("provider card text = %q, want copyable switch commands", text)
	}
}

func TestRenderModelCard_IncludesHintAndCopyableCommands(t *testing.T) {
	e := NewEngine("test", &stubModelModeAgent{}, []Platform{&stubPlatformEngine{n: "test"}}, "", LangEnglish)

	text := e.renderModelCard().RenderText()
	if !strings.Contains(text, "Selection applies immediately") {
		t.Fatalf("model card text = %q, want interaction hint", text)
	}
	if !strings.Contains(text, "/model gpt-4.1") || !strings.Contains(text, "/model gpt-4.1-mini") {
		t.Fatalf("model card text = %q, want copyable model commands", text)
	}
}

func TestRenderReasoningCard_IncludesHintAndCopyableCommands(t *testing.T) {
	e := NewEngine("test", &stubModelModeAgent{}, []Platform{&stubPlatformEngine{n: "test"}}, "", LangEnglish)

	text := e.renderReasoningCard().RenderText()
	if !strings.Contains(text, "Selection applies immediately") {
		t.Fatalf("reasoning card text = %q, want interaction hint", text)
	}
	if !strings.Contains(text, "/reasoning low") || !strings.Contains(text, "/reasoning xhigh") {
		t.Fatalf("reasoning card text = %q, want copyable reasoning commands", text)
	}
}

func TestRenderModeCard_IncludesHintAndCopyableCommands(t *testing.T) {
	e := NewEngine("test", &stubModelModeAgent{}, []Platform{&stubPlatformEngine{n: "test"}}, "", LangEnglish)

	text := e.renderModeCard().RenderText()
	if !strings.Contains(text, "Selection applies immediately") {
		t.Fatalf("mode card text = %q, want interaction hint", text)
	}
	if !strings.Contains(text, "/mode default") || !strings.Contains(text, "/mode yolo") {
		t.Fatalf("mode card text = %q, want copyable mode commands", text)
	}
}

func TestProcessInteractiveEvents_AutoSwitchesProviderOnQuotaError(t *testing.T) {
	p := &stubPlatformEngine{n: "test"}
	sess := newQueuingSession("quota-failover")
	agent := &stubFailoverAgent{
	...
	session.SetAgentSessionID("old-session", "codex")
	session.AddHistory("assistant", "old context")
	...
	sess.events <- Event{Type: EventError, Error: errors.New("429 Too Many Requests: insufficient_quota")}

	select {
	case <-done:
	case <-time.After(3 * time.Second):
		t.Fatal("processInteractiveEvents did not finish after failover")
	}
	...
	e.interactiveMu.Lock()
	_, ok := e.interactiveStates[key]
	e.interactiveMu.Unlock()
	if ok {
		t.Fatal("expected interactive state to be cleaned up after auto failover")
	}
}

func TestSwitchProvider_ResetsConversationState(t *testing.T) {
	p := &stubPlatformEngine{n: "test"}
	agent := &stubProviderAgent{
	...
	session := e.sessions.GetOrCreateActive(msg.SessionKey)
	session.SetAgentSessionID("old-session", "codex")
	session.AddHistory("assistant", "old response")

	e.switchProvider(p, msg, agent, "secondary")

	if agent.active != "secondary" {
		t.Fatalf("active provider = %q, want secondary", agent.active)
	}
	if session.GetAgentSessionID() != "" {
		t.Fatalf("agent session id = %q, want cleared", session.GetAgentSessionID())
	}
