...
Provider 还支持以下自动切号元数据：

- `priority`：自动切号时优先级，数值越大越优先
- `cooldown_sec`：触发配额 / 429 后进入冷却的秒数
- `auto_failover`：设为 `false` 可禁用该 Provider 的自动切换
...
对于 Codex 项目，如果没有显式配置 `model`，cc-connect 会优先选择 `gpt-5.4`，然后依次回退到 `gpt-5.4-codex`、`gpt-5.3-codex`、`o3`。如果没有配置 `reasoning_effort`，默认使用 `xhigh`。
