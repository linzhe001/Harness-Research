...
cc-connect provider add --project my-backend --name codex-primary --api-key sk-xxx --priority 100 --cooldown-sec 1800 --auto-failover=true
...
/provider add {"name":"codex-primary","api_key":"sk-xxx","priority":100,"cooldown_sec":1800,"auto_failover":true}
/provider remove <name>
/provider switch <name>
/provider <name>            Shortcut for switch
```

Providers support optional failover metadata:

- `priority`: higher value wins during automatic failover
- `cooldown_sec`: how long a provider stays on cooldown after a quota / 429 error
- `auto_failover`: set to `false` to disable automatic fallback for a provider
...
For Codex projects, if no explicit `model` is configured, cc-connect now prefers `gpt-5.4`, then `gpt-5.4-codex`, then `gpt-5.3-codex`, then `o3`. If no `reasoning_effort` is set, the default is `xhigh`.
