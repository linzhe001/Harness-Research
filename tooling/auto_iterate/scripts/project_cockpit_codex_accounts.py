#!/usr/bin/env python3
"""Legacy helper for projecting Cockpit Codex accounts into CODEX_HOME dirs.

New auto-iterate runs use external current-auth mode instead. Keep this helper
only for older workspaces that still use the legacy controller account pool.
...
def _resolve_cockpit_dir() -> Path:
    ...
    if target is not None:
        ...
        #   -> /mnt/c/Users/<user>/.antigravity_cockpit
...
def _write_accounts_yaml(path: Path, projected: list[dict[str, str]]) -> None:
    lines = [
        ...
        "# These paths point to local credential projections; do not commit",
        "# machine-specific edits blindly.",
        "accounts:",
    ]
...
def project_accounts(
    ...
    for summary in summaries:
        ...
        _write_json_atomic(
            dest / ".cockpit_codex_auth.json",
            _projection_marker(account),
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--config-template",
        type=Path,
        default=_default_config_template(),
    )
