...
are read from environment variables and are never written to request artifacts.
"""
...
def load_provider_config(
    ...
    *,
    ...
    api_key_env = _required_str(merged, "api_key_env", provider)
...
def invoke_chat_completion(
    ...
    *,
    ...
    if not api_key.strip():
        raise ModelApiError(f"Environment variable {provider.api_key_env} is empty")
    ...
    try:
        ...
        raise ModelApiError(f"HTTP {exc.code} from {provider.name}: {body}") from exc
    except urllib.error.URLError as exc:
        raise ModelApiError(f"Request to {provider.name} failed: {exc}") from exc
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        response = invoke_chat_completion(provider, api_key=api_key, payload=payload)
...
def _required_str(data: dict[str, Any], key: str, provider: str) -> str:
    value = data.get(key)
    if not isinstance(value, str) or not value.strip():
        raise ModelApiError(f"Provider {provider!r} needs {key}")
    return value.strip()
