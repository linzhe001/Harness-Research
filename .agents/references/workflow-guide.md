...
本文档详细介绍基于 Codex Skills 构建的 CV 研究工作流系统。
...
系统采用 Codex 的三层配置机制，按加载频率和作用范围分层：

| 层级 | 位置 | 加载方式 | 用途 |
|------|------|---------|------|
| **AGENTS.md / CLAUDE.md** | 项目根目录 | 会话读取 | 全局上下文：项目名、环境、技术栈、当前阶段 |
| **Shared refs** | `.agents/references/` | skill 按需读取 | 行为约束：代码修改规范、训练前 git 操作、依赖变更提醒 |
| **Skills** | `.agents/skills/` | 用户通过 `$skill-name` 或自然语言调用 | 阶段执行器：每个工作流阶段的完整逻辑 |

**设计理念**：AGENTS.md / CLAUDE.md 保持极简，只放"每次都需要知道的"稳定信息；shared references 按需加载，避免无关信息污染上下文；skills 按需调用，执行具体工作。
...
│  │                      WF8 迭代                         │              │
│  │                                                       │              │
│  │  $iterate plan → code → run → eval → 决策           │              │
│  │      ↑                                     │          │              │
│  │      │              DEBUG                  │          │              │
│  │      └─────────────────────────────────────┘          │              │
│  │                                                       │              │
│  │    可选: $iterate ablate (组件贡献度分析)               │              │
│  └───────────────────────┬───────────────────────────────┘              │
│                          │                                              │
│           ┌──────────────┼──────────────┐                              │
...
$iterate code "升级 backbone 到 ResNet-101"
  → 写入持久化 context (.agents/state/iterations/iter{N}/context.json)
...
**其他关键特性**：
- **持久化 context**: 存储到 `.agents/state/iterations/iter{N}/context.json`
...
**Operation modes**（由 `.agents/state/current_iteration.json` context 决定）：
...
```
                    ...
                    │  调用 $code-debug 修改代码      │  → .agents/state/iterations/iter{N}/
