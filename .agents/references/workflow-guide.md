...
| `project_map.json` | generated by build-plan; updated by code-expert, code-debug, or any workflow step that adds/removes/renames stable files or changes stable interfaces | Stable implementation structure (single source of truth for durable files, not the architecture decision) |
...
| WF8 code-expert | `$code-expert` | Full project codebase |
...
**Per-iteration report**: when invoked from `$iterate eval`, write to `docs/iterations/iter{N}.md` for legacy compatibility and optionally mirror to `docs/40_iterations/iter{N}.md`.
