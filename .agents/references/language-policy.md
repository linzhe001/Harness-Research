...
- `interaction_language`: language used in assistant replies, status updates, and questions to the user.
- `artifact_language`: language used for natural-language sections in generated reports, docs, and summaries.

## Defaults

- Default `interaction_language` to the user's latest substantive input.
- Default `artifact_language` to `interaction_language`.
- If the user explicitly asks for a different output language, follow that request.
